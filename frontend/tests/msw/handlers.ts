...
const mockShipments = [
  {
    ...
    origin: "Bang Rak, Bangkok",
    destination: "Sri Phum, Chiang Mai",
...
const mockAnalytics = {
  ...
  by_region: [
    { name: "Bangkok", total: 40 },
    { name: "Chiang Mai", total: 20 },
  ],
};
  ...
  http.get(`${BASE}/shipments`, ({ request }) => {
    ...
    if (limit === "-1")
      return HttpResponse.json({
        data,
        pagination: { page: 1, limit: -1, total: data.length, totalPages: 1 },
      });
    return HttpResponse.json({
      data,
